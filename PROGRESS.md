...
### 4단계: 문제 제기 및 솔루션 섹션 구현 (현재)
- **섹션 2 (문제 제기)**: 스포트라이트 배경, 모의 브라우저 레이아웃(CVR/광고비 플로팅 카드), 타이포그래피 계층 최적화.
- **섹션 3 (솔루션)**: 칠흑/딥 네이비(`bg-[#030513]`) 기반 초고대비 하이엔드 톤앤매너 배경, 은은한 도트 및 후광(Ambient Glow) 추가. 기존의 오작동 요소를 제거하고 완벽한 시각적 안정감 확보.
- **통일감 반영**: 각 섹션 상단 통일된 캡션(Eyebrow 아이브로우)을 인디고 컬러로 공통 적용하여 일관적인 시선 흐름 추가.
...
│   ├── sections/
│   │   ├── ProblemSection.tsx  # ★ 섹션 2: 문제 제기 영역
│   │   └── SolutionSection.tsx # ★ 섹션 3: 해결책(솔루션) 영역
...
- [x] 히어로 섹션 아래 추가 섹션 구현 (문제 제기, 솔루션 섹션 완료)
- [ ] 마케팅 확장/증명, 포트폴리오, 가격, FAQ 등 후속 섹션 구현
